...
for link in links:
    try:
        ...
        driver.get(link)
        sleep(2)
        Accept_button = driver.find_elements_by_xpath("//button[contains(@class,'artdeco-button artdeco-button--2 artdeco-button--secondary ember-view invitation-card__action-btn')]")
        if (Accept_button==0):
            print("Already Done!")
        else:    
            for accept in Accept_button:
                accept.click()
            print("Accepted")
